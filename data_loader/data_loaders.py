...
#         unique_cells = list(["OVCAR8","22RV1","786O","HT29","A375","K562","A549","JURKAT","MELJUSO"])
        unique_cells = list(["OVCAR8","22RV1","786O","HT29","A375","K562","A549","JURKAT","MELJUSO", "HS944T", "HS936T", "HSC5", "IPC298", "MEL202", "PATU8988S", "PK1", "GI1","SAOS2"])
#         if test_mode == "C1": 
        if cell_list is not None and len(cell_list) > 0:
            if test_mode != "ALL":
...
#         print("CCLE cell line names are {}".format(self.CCLE_exp.index))
        ...
        if self.network_node_embdings != "":
            ...
            self.network_node_embeddings = None
...
#         self.all_CCLE_genes = list(set(self.CCLE_exp.columns) & set(self.CCLE_ess.columns))
    ...
    def _mapping_data(self):
        ...
        # self.network_node_embdings['gene'] = self.network_node_embdings['gene'].apply(lamxbda x: self.gene_mapping[x])
    ...
    def _generate_fixed_network_data(self):
        ...
        if self.network_node_embdings != "":
            self.network_node_feats = np.zeros((len(self.all_genes), 384))#256))#128))
            ...
            self.network_node_feats = np.zeros((len(self.all_genes), 256))#256))#128))
            for gene, idx in self.gene_mapping.items():
                if gene in self.network_node_feats_df.index:
...
#             print('cell name ',cell)
        ...
        specific_omics_1 = torch.tensor([self.CCLE_exp.loc[cell, gene1], self.CCLE_ess.loc[cell, gene1]], dtype=torch.float) #self.CCLE_ess.loc[cell, gene1], self.CCLE_cn.loc[cell, gene1]
...
class CellDataLoader(DataLoader):
    ...
    def __init__(self, data_dir, batch_size, shuffle, validation_split, split_method, num_workers, input_file, all_cells, valid_cells, test_cells, CCLE_exp_file, 
        ...
        self.dataset = MyDataset(data_dir, input_file, CCLE_exp_file, CCLE_ess_file, CCLE_cn_file, cell_embeds_exp, cell_embeds_ess, network_type,
                                                                                                        gene_embeds_combined, network_node_embdings, task, thres, network_specific, train_list=train_list)
