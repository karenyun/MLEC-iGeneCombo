...
class MyDataset(Dataset):
    def __init__(self, data_dir, input_file, CCLE_exp_file, CCLE_ess_file, CCLE_cn_file, cell_embeds_exp, cell_embeds_ess, network_type,
                                                                            gene_embeds_combined, task, thres, network_specific, cell_list=None, train_list=None, test_mode=None, kegg_genes_file=None):
...
#         self.all_CCLE_genes = list(set(self.CCLE_exp.columns) & set(self.CCLE_ess.columns))
        # add cn
        self.all_CCLE_genes = list(set(self.CCLE_exp.columns) & set(self.CCLE_ess.columns)) #& set(self.CCLE_cn.columns))
    ...
    def _generate_fixed_network_data(self):
        ...
        self.network_node_feats = np.zeros((len(self.all_genes), 256))#128))
        for gene, idx in self.gene_mapping.items():
            if gene in self.network_node_feats_df.index:
                self.network_node_feats[idx] = self.network_node_feats_df.loc[gene]
...
#             print('cell name ',cell)
        ...
        cell_input_exp = torch.tensor(self.cell_exp.loc[cell], dtype=torch.float)
        cell_input_ess = torch.tensor(self.cell_ess.loc[cell], dtype=torch.float)
        specific_omics_1 = torch.tensor([self.CCLE_exp.loc[cell, gene1], self.CCLE_ess.loc[cell, gene1]], dtype=torch.float)
        specific_omics_2 = torch.tensor([self.CCLE_exp.loc[cell, gene2], self.CCLE_ess.loc[cell, gene2]], dtype=torch.float)
...
class CellDataLoader(DataLoader):
    ...
    """
    def __init__(self, data_dir, batch_size, shuffle, validation_split, split_method, num_workers, input_file, all_cells, valid_cells, test_cells, CCLE_exp_file, 
                                            CCLE_ess_file, CCLE_cn_file, cell_embeds_exp, cell_embeds_ess, network_type, gene_embeds_combined, task, thres, network_specific):
        train_list = list(set(all_cells)-set(test_cells))
        self.dataset = MyDataset(data_dir, input_file, CCLE_exp_file, CCLE_ess_file, CCLE_cn_file, cell_embeds_exp, cell_embeds_ess, network_type,
                                                                                                        gene_embeds_combined, task, thres, network_specific, train_list=train_list)

#         np.random.seed(12345)
        
        if split_method == "split_by_cell":
            train_cells = list([x for x in all_cells if x not in test_cells and x not in valid_cells])
            self.train_indices = np.where(self.dataset.effect_df["cell"].isin(train_cells))[0]
            self.valid_indices = np.where(self.dataset.effect_df["cell"].isin(valid_cells))[0]
        elif split_method == "split_by_pair":
#             all_samples = train_cells + valid_cells
            all_samples = list([x for x in all_cells if x not in test_cells])
            print(all_samples)
            idx_full = np.where(self.dataset.effect_df["cell"].isin(all_samples))[0]
            n_samples = len(idx_full)
            print("total training data is {}".format(n_samples))
            print("number of samples of different cell-lines {}".format(self.dataset.effect_df[self.dataset.effect_df["cell"].isin(all_samples)]["cell"].value_counts()))
            np.random.shuffle(idx_full)
 
            len_valid = int(n_samples * validation_split)

            self.valid_indices = idx_full[:len_valid]
            self.train_indices = idx_full[len_valid:]
    ...
    def split_validation(self):
        ...
        return DataLoader(sampler=valid_sampler, **self.init_kwargs)
...
class DataLoaderPredict(DataLoader):
    ...
    def __init__(self, data_dir, batch_size, shuffle, validation_split, split_method, num_workers, input_file, test_cells, CCLE_exp_file, 
                                                CCLE_ess_file, CCLE_cn_file, cell_embeds_exp, cell_embeds_ess, kegg_genes_file, network_type, gene_embeds_combined, task, thres, network_specific, test_mode):
        #debug
...
#         cell_list = list(["22RV1"])
        ...
        self.dataset = MyDataset(data_dir, input_file, CCLE_exp_file, CCLE_ess_file, CCLE_cn_file, cell_embeds_exp, cell_embeds_ess, network_type,
                                                                                    gene_embeds_combined, task, thres, network_specific, cell_list, test_mode=test_mode, kegg_genes_file=kegg_genes_file)
        self.task = task
